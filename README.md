...
> 提示：当前仓库仅保留 OKX 全仓监控脚本 `chua_ok_all.py` 与通用风控模块 `risk_utils.py`，其他交易所脚本已经下线。
...
> Binance / Bitget 配置段落仅保留作历史参考；当前运行流程仅依赖 OKX 区段与 `chua_ok_all.py` 中的全仓监控逻辑。
...
- **stop_loss_pct**: 止损百分比，表示当亏损达到该百分比时平仓，例如 2 表示亏损 2% 时平仓。也支持以 `<周期>ATR` 格式配置，例如 `32ATR` 表示使用 15 分钟 32 周期 ATR 的两倍作为止损价差。
